import pandas as pd #Used for: Reading Excel files,Data aggregation,Grouping and calculations.
import os #Used for: Listing files in directories,Building file paths.
from datetime import datetime #Imported for date handling(Not directly used later, but acceptable utility import)
...
def generate_monthly_report(subject, month_year, attendance_dir='../data'):
    ...
    """
    all_data = [] # Empty list.Will store attendance DataFrames from multiple days
    session_dates = set() #Empty set.Stores unique attendance dates.Used to calculate total sessions in the month.
    ...
    for file in os.listdir(attendance_dir):
        ...
        # Filters only:Correct subject.Excel attendance files.
        if file.startswith(f"attendance_{subject}_") and file.endswith(".xlsx"):
            try:
                date_str = file.split('_')[-1].replace('.xlsx', '') # Extract the date part from the filename.
